...
s_min_folding::s_min_folding (std::string seq,char *sequence) : params_(scale_parameters())
...
s_min_folding::s_min_folding (std::string seq,char *sequence, char *restricted) : params_(scale_parameters())
...
// allocate the necessary memory
{
    ...
    V = new s_energy_matrix (seq_, nb_nucleotides,params_);
    ...
    if(cur_interval->type == LOOP)
    {
        ...
        else if (type == INTER)
        {
            ...
            for (ip = i+1; ip <= max_ip; ip++)
            {
                ...
                {
                    tmp = V->compute_int(i,j,ip,jp,params_);
                    if (tmp < min)
                    {
                        min = tmp;
                        best_ip = ip;
