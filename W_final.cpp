...
int W_final::compute_W_br2_restricted_pmo (int j, str_features *fres, int &must_choose_this_branch) {
    ...
    for (i=0; i<=j-1; i++) {
        ...
        if (fres[i].pair <= -1 && i+1 < j)
        {
            ...
            if (energy_ij < INF)
            {
                tmp = energy_ij + acc + (PARAMTYPE) round(pmo_percentage*AU_penalty_pmo(int_sequence[i+1],int_sequence[j]) + rna_percentage*AU_penalty(int_sequence[i+1],int_sequence[j]));
        ...
        if (fres[i].pair <= -1 && fres[j].pair <= -1 && i+1 < j-1)
        {
            ...
            if (energy_ij < INF)
            {
                tmp = energy_ij + acc + (PARAMTYPE) round(pmo_percentage*AU_penalty_pmo(int_sequence[i+1],int_sequence[j-1]) + rna_percentage*AU_penalty(int_sequence[i+1],int_sequence[j-1]));
...
//  else if(cur_interval->type == M_WM)
		{
			...
			if (fres[j].pair <= -1)
			{
				tmp = vm->get_energy_WM (i,j-1) + (PARAMTYPE) round(pmo_percentage*misc_pmo.multi_free_base_penalty + rna_percentage*misc.multi_free_base_penalty);
				if (tmp < min)
				{
					min = tmp;
					best_row = 6;
